 ...
 */
export const calculateDifficulty = (top10AppsData: AppData[], keyword: string): number => {
    ...
    top10AppsData.forEach(app => {
        totalKeywordAppearances += countKeywordAppearances(app.name + ' ' + app.subtitle, keyword);
...
export const calculateDifficultyEx = (top10AppsData: AppData[], keyword: string): number => {
    ...
    top10AppsData.forEach(app => {
        totalKeywordAppearances += countKeywordAppearances(app.name + ' ' + app.subtitle, escapedKeyword);
        totalLogRatingCount += Math.log1p(app.ratingCount);
        totalRating += app.rating;
    });
    ...
    const normalizedKeywordAppearance = (avgKeywordAppearance / maxAvgKeywordAppearance) * 100;
    const normalizedLogRatingCount = (avgLogRatingCount / maxLogRatingCount) * 100;
    ...
    const difficultyScore = (normalizedKeywordAppearance * weightKeywordAppearance +
        normalizedLogRatingCount * weightLogRatingCount +
        normalizedRating * weightRating) / (weightKeywordAppearance + weightLogRatingCount + weightRating);

    return difficultyScore;
};

export const calculateDifficultyEx2 = (top10AppsData: AppData[], keyword: string): number => {
    // Escape special characters in the keyword for regex
    const escapeRegExp = (string: string) => string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const escapedKeyword = escapeRegExp(keyword);
    ...
    top10AppsData.forEach(app => {
        ...
        keywordPresenceCount += checkKeywordPresence(app.name + ' ' + app.subtitle, escapedKeyword);
        totalLogRatingCount += Math.log1p(app.ratingCount);
        totalRating += app.rating;
    });
    ...
    const normalizedKeywordPresence = (avgKeywordPresence / maxKeywordPresence) * 100;
    const normalizedLogRatingCount = (avgLogRatingCount / maxLogRatingCount) * 100;
    const normalizedRating = (avgRating / maxRating) * 100;
    ...
    const difficultyScore = (normalizedKeywordPresence * weightKeywordPresence +
        normalizedLogRatingCount * weightLogRatingCount +
        normalizedRating * weightRating) / (weightKeywordPresence + weightLogRatingCount + weightRating);
